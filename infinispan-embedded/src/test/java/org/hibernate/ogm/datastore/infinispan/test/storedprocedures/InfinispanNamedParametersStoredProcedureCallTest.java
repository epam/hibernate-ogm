...
public class InfinispanNamedParametersStoredProcedureCallTest extends OgmJpaTestCase {
	...
	public void testExceptionWhenUsePositionalParameters() throws Exception {
		...
		thrown.expectMessage( "org.hibernate.HibernateException: OGM001115: Dialect org.hibernate.ogm.datastore.infinispan.InfinispanDialect does not support positional parameters when calling stored procedures" );
	...
	@Test
	public void testExceptionWhenNotACallableProcedure() throws Exception {
		thrown.expect( PersistenceException.class );
		thrown.expectMessage( "org.hibernate.HibernateException: OGM001113: Cannot instantiate stored procedure 'notReallyAProcedure' with resolved name 'org.hibernate.ogm.datastore.infinispan.test.storedprocedures.NotReallyAProcedure'." );
		StoredProcedureQuery storedProcedureQuery = em.createStoredProcedureQuery( "notReallyAProcedure", Void.class );
		storedProcedureQuery.registerStoredProcedureParameter( "param", Integer.class, ParameterMode.IN );
		storedProcedureQuery.setParameter( "param", 1 );
		storedProcedureQuery.getSingleResult();
	}
